...
````````````````````````````````
By default, a FlexiAlertBlock has type "info". In the spec above, the default class and icon for an "info" type FlexiAlertBlock are rendered.
The type of a FlexiAlertBlock, generated classes, icon markup, and more can be customized or omitted - refer to the [options section](#options) for details.
...
[FlexiOptionsBlocks](https://github.com/JeringTech/Markdig.Extensions.FlexiBlocks/blob/master/specs/FlexiOptionsBlocksSpecs.md#flexioptionsblocks) extension must be enabled. To specify default FlexiAlertBlockOptions for all FlexiAlertBlocks,
...
- `Type`
  ...
  - Usage:
    ...
    ````````````````````````````````
    A FlexiAlertBlock's type can be any string containing only valid CSS class characters. By default, a FlexiAlertBlock's type is used to generate its outermost element's class as well as to retrieve
    its icon markup. Icon markups for custom types can be specified using [FlexiAlertBlocksExtensionOptions](#flexialertblocksextensionoptions).
...
- `IconMarkups`
  ...
  - Default: Contains icon markups for types "info", "warning" and "critical-warning".
  - Usage:
    ```````````````````````````````` none
    --------------- Extra Extensions ---------------
    FlexiOptionsBlocks
...
- `DefaultBlockOptions`
  ...
  - Usage:
    ...
    The default FlexiAlertBlockOptions has lower precedence than block specific options:
    ```````````````````````````````` none
    --------------- Extra Extensions ---------------
    FlexiOptionsBlocks
    --------------- Extension Options ---------------
